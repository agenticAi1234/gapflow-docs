---
title: Loops
description: Creating iterative processes with loops in Gapflow
---
...
Loops are a powerful feature in Gapflow that allow you to create iterative processes, implement feedback mechanisms, and build more sophisticated workflows.
...
Loops in Gapflow allow a group of blocks to execute repeatedly, with each iteration building on the results of the previous one. This enables:
...
## Loop types: `for` and `forEach`

Structural loops in the editor are stored on the workflow with a **loop type**:

- **`for`** — run up to **N** iterations (fixed or bounded), with optional conditions to stop early.  
- **`forEach`** — iterate over a **collection** of items; each iteration can expose a **current item** (and index) to blocks inside the loop via loop-aware references (for example current-item bindings in the executor).

Both types share the same execution pipeline: the **loop manager** advances iterations, may **reset block state** between passes, and activates paths when the loop completes.

<Callout type="info">
  A **block-based “loop over items”** UX may be introduced alongside these structural loops; structural **for/forEach** loops remain supported for backward compatibility and complex graphs.
</Callout>
...
By mastering loops, you can create much more sophisticated and powerful workflows in Gapflow.
